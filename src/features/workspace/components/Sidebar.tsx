...
function Sidebar(props: SidebarProps) {
  ...
  useEffect(() => {
    if (!isCreateDialogOpen && movePath === null) return;
    const frame = requestAnimationFrame(() => {
      inputRef.current?.focus();
      inputRef.current?.select();
    });
    return () => cancelAnimationFrame(frame);
  }, [isCreateDialogOpen, movePath]);
  ...
  return (
    <aside className="sidebar" aria-label="File explorer">
      ...
      <div className="explorer-header"><span>Pages</span><button disabled={props.isBusy} onClick={() => setCreateTitle("Untitled")} title="Create page" type="button">+</button></div>
