...
function groupContext(workspace: AiWorkspace, id: "left" | "right") {
  ...
  return {
    ...
    activeFolder: (() => {
      const path = folderPathFromTabId(group.activePath ?? "");
      if (path == null) return null;
      const folder = workspace.project.folders.find((candidate) => candidate.path === path);
      return folder ? { path, title: folder.title } : { path, title: path || "Pages" };
    })(),
